# Q2: Sum Nums
def sum_nums(s):
    """
    Returns the sum of the elements in s.

    >>> a = Link(1, Link(6, Link(7)))
    >>> sum_nums(a)
    14
    """
    "*** YOUR CODE HERE ***"
    # iteration
    sum = 0
    while s != Link.empty:
        sum += s.first
        s = s.rest
    return sum

    # recursion
    # if s is Link.empty:
    #    return 0
    # return s.first + sum_nums(s.rest)

# Q3: Remove All


def remove_all(link, value):
    """Removes all nodes in link that contain value. The first element of
    link is never equal to value.

    >>> l1 = Link(0, Link(2, Link(2, Link(3, Link(1, Link(2, Link(3)))))))
    >>> print(l1)
    <0 2 2 3 1 2 3>
    >>> remove_all(l1, 2)
    >>> print(l1)
    <0 3 1 3>
    >>> remove_all(l1, 3)
    >>> print(l1)
    <0 1>
    >>> remove_all(l1, 3)
    >>> print(l1)
    <0 1>
    """
    "*** YOUR CODE HERE ***"
    if link is Link.empty or link.rest is Link.empty:
        return  # return nothing
    elif link.first != value:
        return Link(link.first, remove_all(link.rest, value))
    else:
        return remove_all(link.rest, value)

    # Official Solution (not sure)
    # if link is Link.empty or link.rest is Link.empty:
    #    return
    # if link.rest.first == value:
        # skip second element
    #    link.rest = link.rest.rest
    #    remove_all(link, value)
    # else:
    #    remove_all(link.rest, value)

    # A different recursive solution
    # if link is not Link.empty and link.rest is not Link.empty:
    #    remove_all(link.rest, value)
    #    if link.rest.first == value:
    #        link.rest = link.rest.rest

    # An iterative solution
    # ptr = link
    # while ptr is not Link.empty and ptr.rest is not Link.empty:
    #    if ptr.rest.first == value:
    #        ptr.rest = ptr.rest.rest
    #    else:
    #        ptr = ptr.rest


# Q4: Flip Two
def flip_two(s):
    """
    Flips every pair of values in s.

    >>> one_lnk = Link(1)
    >>> flip_two(one_lnk)
    >>> one_lnk
    Link(1)
    >>> lnk = Link(1, Link(2, Link(3, Link(4, Link(5)))))
    >>> flip_two(lnk)
    >>> lnk
    Link(2, Link(1, Link(4, Link(3, Link(5)))))
    """
    "*** YOUR CODE HERE ***"
    # 可以透過調整 link 或是調整 node value 的方式去 flip
    # Base case
    if s is Link.empty or s.rest is Link.empty:
        return
    # swap first two value
    s.first, s.rest.first = s.rest.first, s.first
    flip_two(s.rest.rest)

    "*** YOUR CODE HERE ***"
    # iteration
    while not s is Link.empty and not s.rest is Link.empty:
        s.first, s.rest.first = s.rest.first, s.first
        s = s.rest.rest
    return  # 也可以不 return
# Q5: Make Circular


def make_circular(s):
    """Mutates linked list s into a circular linked list.

    >>> lnk = Link(1, Link(2, Link(3)))
    >>> make_circular(lnk)
    >>> lnk.rest.first
    2
    >>> lnk.rest.rest.first
    3
    >>> lnk.rest.rest.rest.first
    1
    >>> lnk.rest.rest.rest.rest.first
    2
    """
    "*** YOUR CODE HERE ***"
    last = s
    while last.rest != Link.empty:
        last = last.rest
    last.rest = s

# Link class


class Link:
    empty = ()

    def __init__(self, first, rest=empty):
        assert rest is Link.empty or isinstance(rest, Link)
        self.first = first
        self.rest = rest

    def __repr__(self):
        if self.rest is not Link.empty:
            rest_repr = ', ' + repr(self.rest)
        else:
            rest_repr = ''
        return 'Link(' + repr(self.first) + rest_repr + ')'

    def __str__(self):
        string = '<'
        while self.rest is not Link.empty:
            string += str(self.first) + ' '
            self = self.rest
        return string + str(self.first) + '>'
