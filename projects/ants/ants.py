...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # ant already there -> with place.ant ; new ant -> self
            if place.ant.is_container and place.ant.ant_contained == None and self.is_container == False:
                # container ant(original) contain the ant added
                place.ant.store_ant(self)
            elif self.is_container and place.ant.is_container == False:
                self.store_ant(place.ant)
                # 把 place.ant 的 attribute 變成 container ant(self) 的 attribute
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # Starting at the current place, keep looking for bees until the hive.
        # If we find a bee or a plenty of bees, return one of them by random_bee()
        place = self.place
        transition = 0
        while place.is_hive != True:
            if place.bees != [] and self.lower_bound <= transition <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # 如果 ant or bee 血量 <=0 ,reduce_health 會自己把它移掉，所以程式裡面可以不用寫移除的部分
                    # if bee.health <= 0:
                    #     place.remove_insect()
...
class WallAnt(Ant):
    ...
    def __init__(self, health=4, place=None):
        """Create an Insect with a health amount and a starting PLACE."""
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        self.health = 1
        self.turns_to_chew = 0  # 在 init 裡面設置 instance attribute
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_chew == 0:
            ...
            # place 沒有 bee, turns_to_chew 維持不變
            if bee == None:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        # 用 other 的 is_container 判斷是否為 ContainerAnt
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        place = self.place
        # TankAnt 的 action 繼承 ContainerAnt 的 action method
        super().action(gamestate)

        # TankAnt 會對 place.bees 傷害 1 點
        for bee in place.bees[:]:
            bee.reduce_health(self.damage)
        # Ant.reduce_health(self, self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        # Follower 的加成攻擊，只看 QueenAnt 後面的格子，不看自己本身這格
        pos = self.place.exit
        while pos:
            if pos.ant is not None:
                if not pos.ant.Double_damage and pos.ant.name != 'Queen':
                    # 先把後面的螞蟻傷害加倍，但是後面的螞蟻還沒有要攻擊
                    pos.ant.double()
                    pos.ant.Double_damage = True
                if pos.ant.is_container and pos.ant.ant_contained is not None:
                    # the pos.ant.ant_contained may change
                    if not pos.ant.ant_contained.Double_damage:
                        pos.ant.ant_contained.double()
                        pos.ant.ant_contained.Double_damage = True
            pos = pos.exit

        # place = self.place.exit
        # while place != None and place.ant:
        #     if not place.ant.Double_damage:
        #         place.ant.double()
        #         place.ant.Double_damage == True
        #     elif place.ant.is_container and place.ant.ant_contained is not None:
        #         if not place.ant.Double_damage:
        #             place.ant.ant_contained.double()
        #             place.ant.ant_contained.Double_damage == True
        #     place = place.exit
